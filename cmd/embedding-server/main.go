...
func main() {
	...
	existingConfig := pkgconfig.ReadEnvConfig(configPath)
	...
	// Only override from existingConfig if env var is non-empty
	if host := os.Getenv("EMBEDDING_SERVER_HOST"); host != "" {
		config.Host = host
	} else if v := existingConfig["EMBEDDING_SERVER_HOST"]; v != "" {
		config.Host = v
	}
	if config.Host == "" {
		config.Host = pkgconfig.DefaultEmbeddingConfig().Host
		if config.Host == "" {
			config.Host = "0.0.0.0"
		}
	}
	...
	// Only override from existingConfig if env var is non-empty
	if host := os.Getenv("LLAMA_SERVER_HOST"); host != "" {
		config.LLMHost = host
	} else if v := existingConfig["LLAMA_SERVER_HOST"]; v != "" {
		config.LLMHost = v
	}
	if config.LLMHost == "" {
		config.LLMHost = pkgconfig.DefaultEmbeddingConfig().LlamaHost
		if config.LLMHost == "" {
	...
	if config.ServerPort == 0 {
		defaults := pkgconfig.DefaultEmbeddingConfig()
		minPort := defaults.PortMin
		maxPort := defaults.PortMax
		if minPort == 0 {
			minPort = 50000
		}
		if maxPort == 0 {
		...
		if err != nil {
			log.Printf("[ERROR] no free embedding server port: %v", err)
			os.Exit(1)
		}
		config.ServerPort = port
	}
	...
	if config.LLMPort == 0 {
		defaults := pkgconfig.DefaultEmbeddingConfig()
		minPort := defaults.LlamaPortMin
		maxPort := defaults.LlamaPortMax
		if minPort == 0 {
			minPort = 18000
		}
		if maxPort == 0 {
		...
		port, err := findFreePort(minPort, maxPort)
		if err != nil {
			log.Printf("[ERROR] no free llama server port: %v", err)
			os.Exit(1)
		}
		config.LLMPort = port
	}
	...
	if config.ModelPath == "" {
		// Auto-detect largest .gguf file in models/ directory (>100MB)
		exe, _ := os.Executable()
		exeDir := filepath.Dir(exe)
		modelsDir := filepath.Join(exeDir, "models")
		if modelPath := findLargestGGUF(modelsDir); modelPath != "" {
			config.ModelPath = modelPath
		} else {
			// Fallback to default
			config.ModelPath = filepath.Join(exeDir, "models", "embeddinggemma-300M-Q8_0.gguf")
		}
	}
	...
	// Write env.config
	_ = pkgconfig.MergeEnvConfig(configPath, map[string]string{
	...
	if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
		log.Printf("Embedding service stopped: %v", err)
	}
}
...
func findFreePort(min, max int) (int, error) {
	...
	return 0, fmt.Errorf("no free port available in range %d-%d", min, max)
}
...
// findLargestGGUF finds the largest .gguf file (>100MB) in the given directory
func findLargestGGUF(dir string) string {
	...
	for _, e := range entries {
		...
		if info.Size() > minSize && info.Size() > largestSize {
			largest = filepath.Join(dir, name)
