...
func NewHNSWIndex(cfg HNSWConfig) (*HNSWIndex, error) {
	...
	// Create index
	distCStr := C.CString(cfg.Distance)
	defer C.free(unsafe.Pointer(distCStr))
	...
	if cfg.StoragePath != "" {
		if fi, err := os.Stat(cfg.StoragePath); err == nil {
			if fi.Size() == 0 {
				...
				pathCStr := C.CString(cfg.StoragePath)
				defer C.free(unsafe.Pointer(pathCStr))
				C.faiss_hnsw_load(ptr, pathCStr)
				idx.loaded = true
	...
	log.Printf("[OK] HNSW index created: dim=%d, M=%d, metric=%s", cfg.Dim, cfg.M, cfg.Distance)
...
func (idx *HNSWIndex) Save(path string) error {
	...
	defer C.free(unsafe.Pointer(pathCStr))
	C.faiss_hnsw_save(idx.ptr, pathCStr)
	log.Printf("[OK] HNSW index saved: %s", path)
	return nil
}
...
func (idx *HNSWIndex) Load(path string) error {
	...
	pathCStr := C.CString(path)
	defer C.free(unsafe.Pointer(pathCStr))
	C.faiss_hnsw_load(idx.ptr, pathCStr)
	log.Printf("[OK] HNSW index loaded: %s", path)
	return nil
}
