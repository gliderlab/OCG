...
func (a *ToolAdapter) RegisterPlugin(name string, plugin PluginLoader) error {
	...
	log.Printf("[OK] registered plugin: %s v%s", info.Name, info.Version)
...
func (a *ToolAdapter) UnregisterPlugin(name string) error {
	...
	if err := plugin.Shutdown(); err != nil {
		log.Printf("[WARN] plugin %s shutdown error: %v", name, err)
	}
...
func CreateBuiltinAdapter(workspace string) *ToolAdapter {
	...
	log.Printf("[PKG] Built-in adapter created (tools will be migrated to plugins)")
	return adapter
}
...
func (a *ToolAdapter) GetToolDocumentation() string {
	...
	for _, plugin := range a.plugins {
		info := plugin.PluginInfo()
		// Escape markdown special characters in tool name
		escapedName := escapeMarkdown(info.Name)
		docs += fmt.Sprintf("## %s\n", escapedName)
		docs += fmt.Sprintf("Version: %s\n\n", info.Version)
		// Escape description
		escapedDesc := escapeMarkdown(info.Description)
