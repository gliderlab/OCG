...
func tavilySearch(query string, count int) ([]tavilyResult, error) {
	...
	if apiKey != "" {
		...
		// Otherwise use Brave Search
		return braveSearch(query, count)
	}

	// If no API key, return prompt
	return []tavilyResult{{
		Title:   "Search feature not configured",
		Url:     "",
		Content: "Please set TAVILY_API_KEY or BRAVE_SEARCH_API_KEY environment variable to enable search",
	}}, nil
}
...
func tavilyAPISearch(query string, count int, apiKey string) ([]tavilyResult, error) {
	...
	// FIX: Limit body size and check errors
	body, err := io.ReadAll(io.LimitReader(resp.Body, 2*1024*1024)) // Max 2MB
	if err != nil {
...
func fetchURL(url, extractMode string, maxChars int) (string, error) {
	// FIX: Add timeout to prevent hanging, allow redirects
	client := &http.Client{
	...
	// FIX: Limit body size to prevent memory exhaustion (max 5MB)
