...
func (sm *SessionManager) GetOrCreateSession(key, agentID string) (*Session, error) {
	// First try to read existing session (with read lock)
	sm.mu.RLock()
	if session, ok := sm.sessions[key]; ok {
		sm.mu.RUnlock()
		return session, nil
	...
	// Not found, need to create (upgrade to write lock)
	// Use Lock() to avoid deadlock - CreateSession will also try to lock
	sm.mu.Lock()
	defer sm.mu.Unlock()
	...
	// Create new session (we already hold the lock)
	session := &Session{
		ID:            fmt.Sprintf("sess-%d", time.Now().UnixMilli()),
		Key:           key,
		AgentID:       agentID,
...
func (c *SessionRPCClient) Connect() error {
	// This would use gRPC to connect
