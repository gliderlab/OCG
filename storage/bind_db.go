...
// If bound to another executable, returns error unless rebind flag is provided via env OCG_BIND_REBIND=true.
func BindExecutable(s *Storage, dbPath string) error {
	...
	// mismatch
	rebind := os.Getenv("OCG_BIND_REBIND")
	if rebind != "true" {
		storedPath, _ := s.GetConfig("binding", "exe_path")
		return fmt.Errorf("database bound to another executable: %s (hash=%s), current=%s (hash=%s). Set OCG_BIND_REBIND=true to override",
